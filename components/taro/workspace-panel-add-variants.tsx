...
/**
 * Left sidebar action buttons — vartest5.
 *
 * The finalized V-Tabs panel (icon rail + active section) stays identical.
 * Only the placement of the Add Warehouse / New Comparison actions varies:
 *
 *   1. Footer            — both buttons stacked in the bottom footer (current).
 *   2. Per-tab top       — the active section pins its own matching button at
 *                          the top of the list area (Warehouses → Add Warehouse,
 *                          Comparisons → New Comparison). No footer.
 *   3. Per-list          — the matching button is the last row of the active
 *                          section's list (scrolls with the content). No footer.
 *   4. Rail +            — one contextual "+" icon button at the bottom of the
 *                          icon rail; it creates for whatever tab is active.
 *   5. Contextual footer — footer shows a single button that matches the
 *                          active tab (swaps as you switch tabs).
 *
...
export type AddButtonVariant = 1 | 2 | 3 | 4 | 5;

interface WorkspacePanelAddVariantsProps {
  // App header
...
const VARIANTS: { id: AddButtonVariant; label: string; blurb: string }[] = [
  { id: 1, label: 'Footer', blurb: 'Both buttons stacked at the bottom footer' },
  { id: 2, label: 'Per-tab top', blurb: 'Matching button pinned above the active list' },
  { id: 3, label: 'Per-list', blurb: 'Matching button as the last row of the list' },
  { id: 4, label: 'Rail +', blurb: 'Single contextual + in the icon rail' },
  { id: 5, label: 'Contextual footer', blurb: 'One footer button that follows the active tab' },
];

type Section = 'warehouses' | 'comparisons';
type EditingTarget = { id: string; type: Section };

export function WorkspacePanelAddVariants(props: WorkspacePanelAddVariantsProps) {
  const [variant, setVariant] = useState<AddButtonVariant>(1);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (n >= 1 && n <= 5) setVariant(n as AddButtonVariant);
    };
  ...
  const railButton = (type: Section, label: string, Icon: typeof WarehouseIcon) => (
    <button
  ...
  /** Full-width add button matching the active section. */
  const activeAddButton = section === 'warehouses' ? <AddWarehouseButton /> : <AddComparisonButton />;

  /** Icon-only contextual add button (used by the rail). */
  const contextualAddButton = (
    <button
      onClick={section === 'warehouses' ? props.onAddWarehouse : props.onNewComparison}
      title={section === 'warehouses' ? 'Add Warehouse' : 'New Comparison'}
      className="flex h-9 w-9 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
    >
      {props.isCreatingComparison && section === 'comparisons' ? (
  ...
  return (
    <div className="w-72 border-r border-border bg-[#F4F4F2] flex flex-col">
      ...
      {/* Body: icon rail + active section (+ add button per variant) */}
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex flex-col items-center gap-1 border-r border-border-default p-1.5 shrink-0">
          ...
          {variant === 4 && (
            <>
              <div className="mt-1 w-6 border-t border-border-default" />
              {contextualAddButton}
            </>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          {variant === 2 && <div className="px-2 pt-2 shrink-0">{activeAddButton}</div>}
          <div className="flex-1 min-h-0 overflow-y-auto p-2">
            {variant === 3 ? (
              <div className="space-y-0.5">
                {activeList}
                <div className="pt-1.5">{activeAddButton}</div>
              </div>
            ) : (
              activeList
            )}
      ...
      {/* Footer: variants 1 (both) and 5 (single contextual) */}
      {(variant === 1 || variant === 5) && (
